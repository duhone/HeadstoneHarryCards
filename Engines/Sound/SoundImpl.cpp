...
void SoundImpl::SetDatabase(IDatabase* const _database)
{
	...
	for(int i = 0;i < sounds.size();++i)
	{
		...
		unordered_map<Guid,shared_ptr<Buffer>,GuidHash>::const_iterator iterator = m_buffers.find(bufferID);
...
{
	std::tr1::unordered_map<Guid,shared_ptr<SoundPrototype>,GuidHash>::const_iterator iterator = m_soundPrototypes.find(_id);
...
void SoundImpl::Tick()
{
	...
	int numDeleted = 0;
	for(int i = 0,j = 0; i< m_currentSoundFX.size();++i)
	{
