...
namespace CR
{
	...
	{		
		...
		//	virtual void SetScreenSize(int arg) {screen_size = arg;};
			...
			virtual Quad* CreateQuad(int _zInitial);
			virtual ISweeper* CreateSweeper(int _zInitial);
			float GetFrameTime() {return m_frameTimer.GetLastFrameTime();}
			virtual std::tr1::shared_ptr<IResourceList> CreateResourceList();
		private:
			bool clearscreen;
			int quality;
			bool capture;
