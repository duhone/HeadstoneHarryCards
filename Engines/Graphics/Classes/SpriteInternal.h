...
namespace CR
{
	...
	{		
		...
		public:
			...
			__inline virtual void SetPositionAbsolute(int x,int y)
			{
				x_position = x;
				y_position = y;
			};
			...
			virtual void RotateZ(float _angle) {m_zAngle = _angle;}
			virtual void UOffset(float _u);
			virtual void VOffset(float _v);	
			virtual int NumFrames() {return max_frame+1;}		
		private:
			bool pause;
